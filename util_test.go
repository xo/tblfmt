...
func psqlEncodeAll(w io.Writer, resultSet ResultSet) error {
	...
	if err = psqlEncode(w, resultSet, dsn); err != nil {
		return err
	}
...
// to the writer.
func psqlEncode(w io.Writer, resultSet ResultSet, dsn string) error {
	var err error
	...
	q := fmt.Sprintf(psqlValuesQuery, vals)
